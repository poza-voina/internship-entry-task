НУЖНО СДЕЛАТЬ ETAG, ТЕСТЫ И каждый третий ход игры существует вероятность 10%, problem details N >= 3 размер поля и условия победы задаются через переменные окружения валидация
