...
public class GameBoardEvaluator(BoardDto board) : IGameBoardEvaluator
{
    ...
    public BoardResultDto Move()
    {
        ...
        var countMoves = Board.Moves.Count();

        return new BoardResultDto
        {
            CellValue = ToggleCellValueWithChance(),
            NextMove = ToggleCellValue(Board.NextMove),
            MoveIndex = countMoves,
            Row = Board.Row,
            Column = Board.Column,
            Winner = CheckWin() ?? CheckDraw(countMoves + 1)
        };
    }

    private CellValue ToggleCellValueWithChance() =>
